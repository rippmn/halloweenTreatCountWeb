...
public class TTReportServiceImplTest {
	...
	private TTEvent[] getTestData() throws Exception {
		...
		long baseTime = System.currentTimeMillis()-(3*YEAR)-(4*HOUR);
		...
		dates = new Date[]{new Date(baseTime+(2*YEAR)), new Date(baseTime+YEAR), new Date(baseTime),
				...
				new Date(baseTime+(2*YEAR)+(3*HOUR)), new Date(baseTime+(3*HOUR))};
		
		int[] counts = new int[] { 0, 0, 0, 1, 2, 3, 4, 5, 6 };
...
//		ttes[0] = new TTEvent();
//		ttes[0].setCount(100);
//		ttes[0].setEventDateTime(new Date());
//
//		when(tteService.getAllTTEvents()).thenReturn(getTestData());
//
//		TTReportServiceImpl service = new TTReportServiceImpl();
//		service.setTrickORTreatEventService(tteService);
//		service.initialize();
